...
import { sys_post } from '@/model'
const db = new Database("sys_post")
...
export default async function (ctx: FunctionContext) {
  ...
  if (ctx.method == "GET") {
    ...
    const [id, error] = await db.post(sys_post, ctx.body, userId)
    if (id) {
    ...
    const [status, error] = await db.put(_id, sys_post, data, userId)
    if (status) {
