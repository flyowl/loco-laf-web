...
export default async function main(ctx: FunctionContext) {
  ...
  console.debug('Log[create-asset] body->', ctx.body, " userId->", userId)
  ...
  const { name, is_default = 0, asset_data = "", version = "", description = "" } = ctx.body;
  ...
  // 验证用户是否已存在
  const { total } = await DB.collection(DB_NAME.SYS_ASSETS).where({ name }).count()
  if (total > 0) {
    return Response.failed('物料已经存在', 400)
  }
  ...
  console.debug('Log[create-asset] users->', users)

  const { id: id } = await DB.collection(DB_NAME.SYS_ASSETS).add({
    name,
    is_default,
    data: asset_data,
    version,
  ...
  const data = { id }
  console.debug('Log[create-asset] result->', data)
