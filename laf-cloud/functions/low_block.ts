...
import { sys_block } from '@/model'
const db = new Database("sys_block")
...
export default async function (ctx: FunctionContext) {
  ...
  if (ctx.method == "GET") {
    ...
    const [id, error] = await db.post(sys_block, ctx.body, userId)
    if (id) {
    ...
    const [status, error] = await db.put(_id, sys_block, data, userId)
    if (status) {
