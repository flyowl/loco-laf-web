...
import { sys_assets } from '@/model'
const db = new Database("sys_assets")
...
export default async function (ctx: FunctionContext) {
  ...
  if (ctx.method == "GET") {
    ...
    const [id, error] = await db.post(sys_assets, ctx.body, userId)
    if (id) {
    ...
    const [status, error] = await db.put(_id, sys_assets, data, userId)
    if (status) {
