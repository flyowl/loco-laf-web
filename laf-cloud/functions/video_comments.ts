...
import { video_comments } from '@/video_model'
const db = new Database("video_comments")
...
export default async function (ctx: FunctionContext) {
  ...
  if (ctx.method == "GET") {
    ...
    const [id, error] = await db.post(video_comments
      , ctx.body, userId)
    if (id) {
    ...
    const [status, error] = await db.put(_id, video_comments, data, userId)
    if (status) {
