...
export default async function main(ctx: FunctionContext) {
  ...
  console.debug('Log[create-typed] body->', ctx.body, " userId->", userId)
  ...
  if (total > 0) {
    return Response.failed('typed已经存在', 400)
  }
  ...
  const { id: id } = await DB.collection(DB_NAME.SYS_TYPED).add({
    name,
    typed,
    p_id,
    description,
  ...
  const data = { id }
  console.debug('Log[create-typed] result->', data)
